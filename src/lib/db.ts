...
import {AssociationId, association} from "./univ";
...
export function collectionName(kind: CollectionKind): string
{
    switch ( kind )
    {
        case CollectionKind.User: return process.env.ROLLOUT_COLLECTION_NAME_USER!;
        case CollectionKind.StudentVerification: return process.env.ROLLOUT_COLLECTION_NAME_STUDENT_VERIFICATION!;
        case CollectionKind.Association: return process.env.ROLLOUT_COLLECTION_NAME_ASSOCIATION!;
    }
}

export async function userProps(db: mongo.Db, id: mongo.ObjectId, givenProject: any): Promise<any>
{
    return await absProps(
        db.collection(process.env.ROLLOUT_COLLECTION_NAME_USER!),
        id,
        givenProject,
    );
}

export async function studentVerificationProps(db: mongo.Db, id: mongo.ObjectId, givenProject: any): Promise<any>
{
    return await absProps(
        db.collection(process.env.ROLLOUT_COLLECTION_NAME_STUDENT_VERIFICATION!),
        id,
        givenProject,
    );
}
...
export const studentVerificationStateStringToEnum = {
    PENDING: StudentVerificationState.Pending as number,
    VERIFIED: StudentVerificationState.Verified as number,
    REJECTED: StudentVerificationState.Rejected as number,
}
